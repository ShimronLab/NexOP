...
def coils_sampling(inp: torch.Tensor, csm_c: torch.Tensor, masks: torch.Tensor, k_mag: bool = False, mask_i= 0):
    ...
    """
    #print('kspace shape', kspace_c.shape)
    #print('csm_c shape', csm_c.shape)
    # IFFT each coil
    ...
    #print('img_cplx shape', img_cplx.shape)
    csm_cplx = torch.view_as_complex(csm_c.contiguous())   # [C,H,W]
    #print('csm_cplx shape', csm_cplx.shape)

    #img_coil = (img_cplx * csm_cplx) 
    #print('img_coil shape', img_coil.shape)
    kspace_coil = fastmri.fft2c(torch.view_as_real(img_cplx)) # [H,W,2] (complex-last) 
    #print('kspace_coil shape', kspace_coil.shape)
    #print('masks shape', masks.shape)
    
    #print('masks shape', masks.shape)
    #print('inp shape', inp.shape)
    #masks[:,0,...] = 1 # [B,3,C,H,W,2]
    #masks[:,2,...] = 0 # [B,3,C,H,W,2]
    #masks[:,1,...] = 0 # [B,3,C,H,W,2]
    ...
    #print('image_sampled_cplx shape', image_sampled_cplx.shape)
    ...
    #count = masks.sum(dim=1).clamp(min=eps)         # [B,2,H,W]
    #kspace_out = kspace_sampled[...].sum(dim=1) / count    # [B,2,H,W]
    #kspace_out = kspace_sampled[:,0,...]
    #print("kspace_avg shape", kspace_avg.shape)
    # Build combined mask for MoDL use
    #print('masks shape', masks.shape)
    #combined_mask = (masks.sum(dim=1) > 0).float() 
    #print(f'combined_mask shape: {combined_mask.shape}, acc: {combined_mask[0,0,...,0].sum().item()/(256*195)}')
    #combined_mask = combined_mask[:,:,:,:,1].clone() # same mask for all coils and complex
    combined_mask = masks[...,0]#.clone()
    #print('kspace_sampled shape', kspace_sampled.shape)
    #print('kspace_out shape', kspace_out.shape)
...
def sense_combine_slice(inp: torch.Tensor, csm_c: torch.Tensor, eps: float = 1e-6, k_mag: bool = False):
    ...
    #denom = (csm_cplx * torch.conj(csm_cplx)).sum(dim=1)   # [H,W]
    
    out = num  #   / (denom + eps)                              # [H,W]
